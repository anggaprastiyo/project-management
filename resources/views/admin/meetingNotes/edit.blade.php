...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.meetingNote.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.meeting-notes.update", [$meetingNote->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('project') ? 'is-invalid' : '' }}" name="project_id" id="project_id" required>
                    @foreach($projects as $id => $entry)
                        <option value="{{ $id }}" {{ (old('project_id') ? old('project_id') : $meetingNote->project->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('meeting_date') ? 'is-invalid' : '' }}" type="text" name="meeting_date" id="meeting_date" value="{{ old('meeting_date', $meetingNote->meeting_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('participants') ? 'is-invalid' : '' }}" name="participants[]" id="participants" multiple required>
                    @foreach($participants as $id => $participant)
                        <option value="{{ $id }}" {{ (in_array($id, old('participants', [])) || $meetingNote->participants->contains($id)) ? 'selected' : '' }}>{{ $participant }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('topic') ? 'is-invalid' : '' }}" type="text" name="topic" id="topic" value="{{ old('topic', $meetingNote->topic) }}" required>
                @if($errors->has('topic'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('note') ? 'is-invalid' : '' }}" name="note" id="note">{!! old('note', $meetingNote->note) !!}</textarea>
                @if($errors->has('note'))
