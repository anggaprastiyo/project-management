            ...
            <form method="POST" action="{{ route("admin.projects.update", [$project->id]) }}"
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control {{ $errors->has('team') ? 'is-invalid' : '' }}" name="team"
                                    id="team" {{ !$isProjectAlreadyHaveTicket ? 'disabled' : '' }}>
                                <option value
                                        disabled {{ old('team', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                                @foreach(App\Models\Project::TEAM_SELECT as $key => $label)
                                    <option
                                        value="{{ $key }}" {{ old('team', $project->team) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('team'))
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input class="form-control {{ $errors->has('ticket_prefix') ? 'is-invalid' : '' }}"
                                   type="text" name="ticket_prefix" id="ticket_prefix"
                                   value="{{ old('ticket_prefix', $project->ticket_prefix) }}"
                                   required {{ !$isProjectAlreadyHaveTicket ? 'disabled' : '' }}>
                                ...
                                @foreach($members as $key => $member)
                                    <optgroup label="{{$member['text']}}">
                                        @foreach($member['children'] as $data)
                                            <option
                                                value="{{ $data['id'] }}" {{ (old('project_owner_id') ? old('project_owner_id') : $project->project_owner->id ?? '') == $data['id'] ? 'selected' : '' }}>{{ $data['nik'].' - '.$data['name'].' ('.$data['job_position_text'].')' }}</option>
                                        @endforeach
                                    </optgroup>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select
                                ...
                                @foreach($project_statuses as $id => $entry)
                                    <option
                                        value="{{ $id }}" {{ (old('project_status_id') ? old('project_status_id') : $project->project_status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status_type') ? 'is-invalid' : '' }}"
                            name="status_type" id="status_type" {{ !$isProjectAlreadyHaveTicket ? 'disabled' : '' }}>
                        <option value
                        ...
                        @foreach(App\Models\Project::STATUS_TYPE_SELECT as $key => $label)
                            <option
                                value="{{ $key }}" {{ old('status_type', $project->status_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('members') ? 'is-invalid' : '' }}"
                            name="members[]" id="members" multiple required>
                        @foreach($members as $key => $member)
                            <optgroup label="{{$member['text']}}">
                                @foreach($member['children'] as $data)
                                    <option
                                        value="{{ $data['id'] }}" {{ (in_array($data['id'], old('members', [])) || $project->members->contains($data['id'])) ? 'selected' : '' }}>{{ $data['nik'].' - '.$data['name'].' ('.$data['job_position_text'].')' }}</option>
                                @endforeach
                            </optgroup>
                        @endforeach
                    </select>
