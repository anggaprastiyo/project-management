<div class="m-3">
    @can('ticket_status_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.ticket-statuses.create', ['project_id' => $project->uuid]) }}">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table
                    class=" table table-bordered table-striped table-hover datatable datatable-projectTicketStatuses">
                    <thead>
                    <tr>
                        <th width="10">
                    ...
                    @foreach($ticketStatuses as $key => $ticketStatus)
                        <tr data-entry-id="{{ $ticketStatus->id }}">
                            ...
                            <td>
                                <span class="color-circle" style="background-color:{{ $ticketStatus->color ?? '' }}"></span> {{ $ticketStatus->color ?? '' }}
                            </td>
                            <td>
                                {{ $ticketStatus->order ?? '' }}
                            </td>
                            <td>
                                @can('ticket_status_show')
                                    <a class="btn btn-xs btn-primary"
                                       href="{{ route('admin.ticket-statuses.show', $ticketStatus->uuid) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                ...
                                @can('ticket_status_edit')
                                    <a class="btn btn-xs btn-info"
                                       href="{{ route('admin.ticket-statuses.edit', $ticketStatus->uuid) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('ticket_status_delete')
                                    <form action="{{ route('admin.ticket-statuses.destroy', $ticketStatus->id) }}"
